...
const GroupSkill = ({title , skillArray}:{title:string , skillArray:any}) => {
  return (
    <div className=' dark:hover:bg-[#131313] rounded-[12px] p-6 flex flex-col items-center   shadow-border'>
        <h4>
            {title}
        </h4>
        <div className="grid grid-cols-3 gap-4 mt-10 "> 
            {skillArray.map((skill: any) => (
                <Badge key={skill.id} className={`ml-1 place-content-center text-center p-2 bg-opacity-60 dark:bg-opacity-20 font-light dark:hover:bg-black hover:text-white text-black bg-[#bdbbbb] dark:text-gray-300 dark:bg-[#4d4d4d]`}> {skill.title} </Badge>
            ))}
            </div>
    </div>
  )
