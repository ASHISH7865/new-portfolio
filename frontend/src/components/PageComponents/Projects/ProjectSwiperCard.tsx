  ...
  return (
    <div className="shadow-border  rounded-2xl  ">
      ...
      <div
        ...
        "
        ></div>
        <Image className="rounded-2xl object-cover min-h-[400px] sm:min-h-[600px] " quality={100} src={`${process.env.NEXT_PUBLIC_STRAPI_API_URL }${props?.attributes?.image?.data?.attributes?.url}`} alt={"img"} width={props?.attributes?.image?.data?.attributes?.width} height={props?.attributes?.image?.data?.attributes?.height} />
